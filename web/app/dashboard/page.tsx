    ...
    <div className="min-w-0 flex-1 space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          {alertMsg && <span className="text-sm text-indigo-300">{alertMsg}</span>}
      ...
      {isEmpty ? (
        ...
        <>
          ...
          <Card>
            ...
            <CardBody>
              {riskByBand.length === 0 ? (
                <p className="text-sm text-stone-500">No risk scores computed yet.</p>
              ) : (
                    ...
                    return (
                      <div key={b.band} className="flex items-center gap-3">
                        ...
                        <div className="h-3 flex-1 overflow-hidden rounded-full bg-stone-800">
                          <div
                            className={`h-full rounded-full ${bandColors[band] ?? 'bg-stone-500'}`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                        <div className="w-12 shrink-0 text-right text-sm font-semibold tabular-nums text-stone-200">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="flex items-center justify-between">
                ...
                <Link href="/dashboard/employees" className="text-xs text-indigo-400 hover:text-indigo-300">
                  All employees →
                </Link>
              </CardHeader>
                      ...
                      {topAtRisk.map((e, i) => {
                        ...
                        return (
                          <TR key={id ?? i}>
                            ...
                            <TD className="text-stone-400">{e.department ?? '—'}</TD>
            ...
            <Card>
              <CardHeader className="flex items-center justify-between">
                ...
                <Link href="/dashboard/drivers" className="text-xs text-indigo-400 hover:text-indigo-300">
                  Drivers →
                </Link>
              </CardHeader>
              <CardBody>
                {topDrivers.length === 0 ? (
                  <p className="text-sm text-stone-500">No exit drivers recorded.</p>
                ) : (
                      ...
                      return (
                        <li key={i} className="flex items-center justify-between gap-2">
                          <span className="flex items-center gap-2 text-sm text-stone-200">
                            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-stone-800 text-xs font-semibold text-indigo-400">
                              {i + 1}
                            </span>
                            {label}
                          </span>
          ...
          <Card>
            <CardHeader className="flex items-center justify-between">
              ...
              <Link href="/dashboard/managers" className="text-xs text-indigo-400 hover:text-indigo-300">
                Scorecards →
              </Link>
            </CardHeader>
            <CardBody className="p-0">
              {managerOutliers.length === 0 ? (
                <p className="px-5 py-6 text-sm text-stone-500">No manager outliers detected.</p>
              ) : (
                <Table>
                  <THead>
                    <TR>
                        ...
                        <TD>
                          {m.manager_id ? (
                            <Link
                              ...
                              className="font-medium text-stone-200 hover:text-indigo-300"
                            >
                              {m.manager_name ?? m.manager_id}
                            </Link>
                          ) : (
                            <span className="font-medium text-stone-200">{m.manager_name ?? '—'}</span>
                          )}
                        </TD>
                        <TD className="text-right tabular-nums text-stone-400">{m.team_size ?? '—'}</TD>
